...
ÉTAPE 3 — DONNÉES POUR LES CARTES DU PORTFOLIO
Après le rapport, génère une fiche par actualité de la section 2.
Génère OBLIGATOIREMENT une fiche pour CHAQUE actualité (3 à 5 fiches).

Commence cette section par exactement :
===DEBUT_CARTES===

Pour CHAQUE actualité, génère une fiche en respectant CE FORMAT EXACT
(une fiche par article, séparées par ---CARTE---) :

THEMES: [valeurs parmi : RGPD, Cloud, IA, Cyber, Réglementation, Identité, Infra — séparées par espace]
MOIS: {mois_iso}
SOURCE: [nom exact de la source]
DATE: [JJ mois AAAA en français]
LIEN: [URL directe de l'article]
TITRE: [titre exact de l'article]
RESUME_COURT: [2 à 3 phrases max, accessibles, pour donner envie de cliquer]
DETAIL_DEBUT
[résumé détaillé complet, identique au rapport, avec chiffres et exemples]
DETAIL_FIN

---CARTE---

[répéter ce bloc pour chaque actualité]
...
- Réponds UNIQUEMENT avec le contenu du rapport puis les fiches cartes.
  Pas de phrase d'intro, pas de conclusion après les fiches.
- Les titres, dates, sources et URLs doivent être exacts et vérifiables.
- IMPORTANT : génère TOUJOURS une fiche pour CHAQUE actualité de la section 2,
...
def split_report(report_md: str) -> tuple[str, list[dict]]:
    """
    Sépare le rapport lisible des fiches cartes.
    Retourne (rapport_texte, liste_de_cartes).
    Chaque carte est un dict avec les clés :
      themes, mois, source, date, lien, titre, resume_court, detail
    """
    if "===DEBUT_CARTES===" not in report_md:
        return report_md.strip(), []

    parts = report_md.split("===DEBUT_CARTES===", 1)
    rapport_part = parts[0].strip()
    cartes_raw = parts[1]
    if "===FIN_CARTES===" in cartes_raw:
        cartes_raw = cartes_raw[:cartes_raw.index("===FIN_CARTES===")]

    cartes = []
    for bloc in cartes_raw.split("---CARTE---"):
        bloc = bloc.strip()
        if not bloc:
            continue
        carte = {}
        # Extraire DETAIL entre DETAIL_DEBUT et DETAIL_FIN
        if "DETAIL_DEBUT" in bloc and "DETAIL_FIN" in bloc:
            before, rest = bloc.split("DETAIL_DEBUT", 1)
            detail, _ = rest.split("DETAIL_FIN", 1)
            carte["detail"] = detail.strip()
            bloc = before  # parse les autres champs sur la partie avant DETAIL_DEBUT
        else:
            carte["detail"] = ""
        ...
        for line in bloc.splitlines():
            ...
            if line.startswith("THEMES:"):
                carte["themes"] = line[7:].strip()
            elif line.startswith("MOIS:"):
                carte["mois"] = line[5:].strip()
            elif line.startswith("SOURCE:"):
                carte["source"] = line[7:].strip()
            elif line.startswith("DATE:"):
                carte["date"] = line[5:].strip()
            elif line.startswith("LIEN:"):
                carte["lien"] = line[5:].strip()
            elif line.startswith("TITRE:"):
                carte["titre"] = line[6:].strip()
            elif line.startswith("RESUME_COURT:"):
                carte["resume_court"] = line[13:].strip()

        if carte.get("titre"):  # ne garder que les fiches complètes
            cartes.append(carte)

    return rapport_part, cartes
...
def build_email_html(report_md: str, mois_annee: str, search_count: int) -> tuple[str, str]:
    """
    Retourne (html_du_mail, contenu_txt_piece_jointe).
    La pièce jointe .txt contient les blocs HTML prêts à coller.
    """
    rapport_part, cartes = split_report(report_md)
...
def main() -> None:
    ...
    html_body, txt_attachment = build_email_html(report_md, mois_annee, search_count)
